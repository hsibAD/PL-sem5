...
struct AST {
  ...
  union {
    struct binop {
      enum binop_type { BIN_PLUS, BIN_MINUS, BIN_MUL, BIN_DIV } type; //Добавлен BIN_DIV
...
DECLARE_BINOP(divide, DIV) //И тут Диван, для деления
...
static const char *BINOPS[] = {
    [BIN_PLUS] = "+", [BIN_MINUS] = "-", [BIN_DIV] = "/", [BIN_MUL] = "*"}; //И даже тут есть диван BIN_DIV 
...
int main() {
    struct AST *ast = NULL;
    ast = add(lit(999), lit(728));
    print(stdout, ast);
    printf("\n");

    ast = add(lit(4), mul(lit(2),lit(9)));
    print(stdout, ast);
    printf("\n");

    ast = mul(add(lit(3),lit(5)), divide(lit(9),lit(7)));
    print(stdout, ast);
    printf("\n");
